...
initGUI(pfChannel *masterChan)
{
	...
	pfChanViewport(masterChan, a + 0.20 * (b-a), b, c, d);
    	...
    	{
    	    pfPipe *p = pfGetChanPipe(masterChan);
    	    pfCompositor* comp = pfGetPipeCompositor(p);
    	    pfCompositorViewport(comp, a+0.20*(b-a), b, c, d);
    	}
    } 
    else 
    {
	...
	pfChanViewport(masterChan, a , b, c + 0.20 * (d-c), d);
    	...
    	if (ViewState->master_pipe_is_composited)
    	{
    	    pfPipe *p = pfGetChanPipe(masterChan);
    	    pfCompositor* comp = pfGetPipeCompositor(p);
    	    pfCompositorViewport(comp, a ,b, c+0.20*(d-c),d);
    	}
    }
...
updateGUI(int guiOn)
{
	...
	if(ViewState->guiFormat == GUI_VERTICAL)
	{
	    pfChanViewport(ViewState->masterChan, r, cb, cc, cd);
    	    ...
    	    if (ViewState->master_pipe_is_composited)
    	    {
    		pfPipe *p = pfGetChanPipe(ViewState->masterChan);
    		pfCompositor* comp = pfGetPipeCompositor(p);
    		pfCompositorViewport(comp,  r, cb, cc, cd);
    	    }
	}
	else
	{
	    pfChanViewport(ViewState->masterChan, ca, cb, t, cd);
    		...
    		pfPipe *p = pfGetChanPipe(ViewState->masterChan);
    		pfCompositor* comp = pfGetPipeCompositor(p);
    		pfCompositorViewport(comp,  ca, cb, t, cd);
    	    }
	}
    }
    else    /* turn off GUI */
	    ...
	    ViewState->powerwall.num_composited_rows) > 1 ) )
	{
	    ...
	    int vpwidth = ViewState->maxScreenX/ViewState->powerwall.num_composited_cols;
	    int vpheight = ViewState->maxScreenY/ViewState->powerwall.num_composited_rows;
	    int vpL = (midcol%ViewState->powerwall.num_composited_cols)*vpwidth;
	    int vpB = (midrow%ViewState->powerwall.num_composited_rows)*vpheight;

    	    pfNotify (PFNFY_DEBUG, PFNFY_PRINT, 
    	    	"updateGUI : POWERWALL MASTER CHANNEL [%d,%d] "
		"VIEWPORT: left=%f right=%f bott=%f top=%f",
    	    	    midcol, midrow,
 	    ...
 	    pfChanViewport(ViewState->masterChan, 
    	    	((float)vpL)/(float)(ViewState->maxScreenX), 
    	    	((float)(vpL+vpwidth/*-1*/))/(float)(ViewState->maxScreenX), 
    	    	((float)vpB)/(float)(ViewState->maxScreenY), 
    	    	((float)(vpB+vpheight/*-1*/))/(float)(ViewState->maxScreenY) );
	...
	else if (Multipipe || (ViewState->numChans == 1)) 
	{
	    pfChanViewport(ViewState->masterChan, 0.0f, 1.0f, 0.0f, 1.0f);
    		...
    		pfPipe *p = pfGetChanPipe(ViewState->masterChan);
    		pfCompositor* comp = pfGetPipeCompositor(p);
    		pfCompositorViewport(comp, 0.0f, 1.0f, 0.0f, 1.0f);
    	    }
	}
	...
	else
	{
	    ...
	    pfChanViewport(ViewState->masterChan, ca, cb, cc, cd);
    		...
    		pfPipe *p = pfGetChanPipe(ViewState->masterChan);
    		pfCompositor* comp = pfGetPipeCompositor(p);
    		pfCompositorViewport(comp, ca, cb, cc, cd);
    	    }
	}
    }

    if (Multipipe==POWERWALL)
    	ComputePowerWallFrustums();
}
...
controlPanel(pfuWidget *w)
{
    ...
    case GUI_SMALL_FEATURE_CULL:
	updateSmallFeatureCulling(pfuGetWidgetValue(w));
	break;
    ...
    case GUI_FOV:  
	{
	    ...
	    if (ViewState->numPanoramicElts > 0)
	    {
		/* remove viewport from channel share */
		int share = pfGetChanShare(ViewState->masterChan);
	    ...
	    if (Multipipe == POWERWALL)
	    {
		/* remove fov from channel share */
		int share = pfGetChanShare(ViewState->masterChan);
		share &= ~PFCHAN_VIEWPORT;
		pfChanShare(ViewState->masterChan, (share & ~PFCHAN_FOV) );

		fov = pfuGetWidgetValue(w);

		if (InitFOV==0)
		{
		    ViewState->fov[0] = fov;
		    ViewState->fov[1] = -1.0f;
	    ...
	    else if (InitFOV) /* don't modify the fov */
	    { 
	    ...
	    {
		float width, scaleFactor, height, left, right;
		int share;

		pfNotify(PFNFY_DEBUG, PFNFY_PRINT,
		    "Using user-supplied asymmetric view frustum = %f %f\n",
			ViewState->fov[0], ViewState->fov[1]);

		/* remove fov from channel share */
		share = pfGetChanShare(ViewState->masterChan);
		pfChanShare(ViewState->masterChan, (share & ~PFCHAN_FOV) | PFCHAN_VIEWPORT );

		/* calculate persp numbers
		  this is in screen space */
		width = ( 1.0 - ( 0.01 * ViewState->fov[3] ) ) * ViewState->numChans + ( 0.01 * ViewState->fov[3] );

		/* this is in fov space */
		height = ( pfTan ( ViewState->fov[1] / 2 ) );
		/*  this converts screen space to fov space */
		scaleFactor = ( 2 *pfTan( ( ViewState->fov[0] / 2 ) ) ) / width;

		pfNotify(PFNFY_DEBUG, PFNFY_PRINT, "width is %f, ScaleFactor is %f\n", width, scaleFactor);


		/* iterate to set persp */
		for ( c = 0; c < ViewState->numChans ; c++)
		{
		    ...
		    pfNotify(PFNFY_DEBUG, PFNFY_PRINT,
			"Asymmetric Frustum settings: channel = %d, left = %f, right = %f, bottom = %f, top = %f", 
			    c, left, right, -1.0 * height, height);

		    pfMakePerspChan(ViewState->chan[ChanOrder[c]], left * scaleFactor , right * scaleFactor, -1.0 * height, height);

		    left = right - ( 0.01 * ViewState->fov[3]);
		    right = left+1.0;
		}
	    }
		...
		if(Multipipe != COMPOSITOR && 
		    Multipipe != SOFTWARE_COMPOSITOR &&
		    Multipipe != POWERWALL )
		{
		    if (InitFOV || InitFOVasym) {
		        pfSetVec3(hpr, 
    ...
    case GUI_ZFOOT_PRINT:
        ...
        break;
    case GUI_SHADERS:
        ViewState->shaders = pfuGetWidgetValue(w);
        drawModesChanged();
	break;
...
/*     ViewState->num_widgets++; */
	...
	if (InitFOV)
	{
	    minFOV = PF_MIN2(minFOV, ViewState->fov[0]);
	    maxFOV = PF_MAX2(maxFOV, ViewState->fov[0]);
	}
    ...
    /* Slider to set small feature culling pizel size */
    wid = pfuNewWidget(panel, PFUGUI_SLIDER, GUI_SMALL_FEATURE_CULL);
    pfuWidgetDim(wid, x, y, xSize, ySize);
    pfuWidgetLabel(wid, "Cull smaller than");
    pfuWidgetRange(wid, PFUGUI_FLOAT, 0.0, 80, 0.0);
    pfuWidgetActionFunc(wid, controlPanel);
    ViewState->guiWidgets[GUI_SMALL_FEATURE_CULL] = wid;
    ViewState->guiState[ViewState->num_widgets].pane = GUI_FOV_PANE;
    ViewState->guiState[ViewState->num_widgets].widget = GUI_SMALL_FEATURE_CULL;
    ViewState->guiState[ViewState->num_widgets].across = 1;
    ...
    wid = pfuNewWidget(panel, PFUGUI_SWITCH, GUI_SHADERS);
    pfuWidgetDim(wid, x, y, xSize, ySize);
    pfuWidgetLabel(wid, "Shaders");
    pfuWidgetActionFunc(wid, controlPanel);
    pfuWidgetDefaultValue(wid,ViewState->shaders);
    ViewState->guiWidgets[GUI_SHADERS] = wid;
    ViewState->guiState[ViewState->num_widgets].pane = -1;
    ViewState->guiState[ViewState->num_widgets].widget = GUI_SHADERS;
    ViewState->guiState[ViewState->num_widgets].across = 2;
    ViewState->num_widgets++;

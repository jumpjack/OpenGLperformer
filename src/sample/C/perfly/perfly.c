...
#define strncasecmp _strnicmp
...
#if (defined(__linux__) && !defined(__linux__has_MP__)) || defined(WIN32)
    ...
    ViewState->snapFrame	= -1;
    ViewState->snapFile		= NULL;
    ...
    ViewState->compositor   	= NULL;
    ViewState->compositorMode	= PFCOMP_COMPMODE_BOTT_TILES;
...
localPreFrame(pfChannel *chan)
{
    ...
    if (pfGetNumCompositors()>0)
    {
    	static this_pipe_done = 0;

	pfuGUICursor(PFU_CURSOR_MAIN, PFU_CURSOR_OFF);
	pfuGUICursor(PFU_CURSOR_GUI, PFU_CURSOR_arrow);
	pfuCursorType(PFU_CURSOR_2D);
...
//#ifdef __ia64__
    	if (this_pipe_done==0)
    	{
    	    ...
    	    Window win = pfGetPWinWSDrawable( pfGetChanPWin(chan) );

    	    if (win && dpy)
    	    {
    		XDefineCursor ( dpy, win, 255 );
    	    	this_pipe_done=1;
    	    }
    	}
#endif
   ...
   /*
	...
	if (ViewState->cullDelta < 1.0f)
	{
	    ...
	    pfGetGSetAttrLists(pfGetGSet(ViewState->cullVol, 0), 
    	    ...
    	    /* XXX Paolo: 
    	    	for composited channels, pfGetChanBaseFrust seems to
    	    	return actual frust used instead of nominal frust..
    	    	Use pfChannel::nb_getUserLeftRightBottomTop instead..
    	    */
    	    

    	   /* if (ViewState->master_pipe_is_composited)  */
    	    /* if (ViewState->compositor)*/
    	    {
		float l, r, b, t;

    	    	pfGetChanNearFar(ViewState->masterChan,  &nearPlane, &farPlane);

    	    	if (ViewState->powerwall.numChans > 0)
    	    	{
    	    	    float dummy;
    	    	    pfChannel * pw_chan = ViewState->chan[0];
    	    	    pfUserLeftRightBottomTopChan(pw_chan, &l, &dummy, &b, &dummy );
    	    	    pw_chan = ViewState->chan[ViewState->numChans-1];
    	    	    pfUserLeftRightBottomTopChan(pw_chan, &dummy, &r, &dummy, &t );
    	    	}
    	    	else
    	    	    pfUserLeftRightBottomTopChan(ViewState->masterChan, &l, &r, &b, &t );
    	
    	    	coords[0][0] = l;
    	    	coords[0][1] = nearPlane;
    	    	coords[0][2] = b;

   	    	coords[1][0] = r;
    	    	coords[1][1] = nearPlane;
    	    	coords[1][2] = b;

   	    	coords[2][0] = r;
    	    	coords[2][1] = nearPlane;
    	    	coords[2][2] = t;

   	    	coords[3][0] = l;
    	    	coords[3][1] = nearPlane;
    	    	coords[3][2] = t;
    	    }
#if 0
    	    else
	    ...
	    pfMakePerspFrust(frust, coords[0][0], coords[2][0], 
    	    ...
    	    {
    	    	int pipe_index;
    	    	for (pipe_index=0; pipe_index<NumPipes; pipe_index++)
    	    	    pfChanCullPtope(ViewState->chan[pipe_index], (pfPolytope*)frust);    	    	    
    	    }
    	    else
...
updateStats(void)
{
    ...
    */
...
#if 0
    if (ViewState->compositor)
    	n = pfGetCompositorNumPipes(ViewState->compositor);
    else if (ViewState->powerwall.numChans >0)
    	n = ViewState->powerwall.numChans;
    else
    	n = ViewState->numChans; 
#else
    	n = pfGetMultipipe();
#endif
	...
	for (i=0; i < n; i++)
	{
	    pfChannel *chan = ViewState->chan[i];
...
initPipe(pfPipeWindow *pw)
{
    ...
    ViewState->useOverlay = ViewState->useOverlay & ((pfGetPWinOverlayWin(pw) ? 1 : 0));
...
snapImage(int snapAlpha)
{
    ...
    if (ViewState->snapFile != NULL)
	strcpy(filename, ViewState->snapFile);
    else
    	sprintf(filename, "perfly.%05d.%s", id,
		    (snapAlpha) ? "rgba" : "rgb");
    pfNotify(PFNFY_INFO, PFNFY_PRINT, "Saving pipe %d image in file %s\n",
	    pfGetId(cpipe), filename);

    /* read from FRONT-buffer to get channel stats info */
    pfuSaveImage(filename, 0, 0,
...
localPreDraw(pfChannel *chan, void *data)
{
	...
	/* enable/disable GLSL shaders, ARB vertex shaders and ARB fragment shaders */
	if(ViewState->shaders == 0) 
	{
 	    pfOverride(PFSTATE_ENSHADPROG | PFSTATE_SHADPROG | 
		       PFSTATE_ENVTXPROG  | PFSTATE_VTXPROG  |
		       PFSTATE_ENFRAGPROG | PFSTATE_FRAGPROG, 0);
	    pfDisable(PFSTATE_ENSHADPROG);
	    pfDisable(PFSTATE_SHADPROG);
	    pfDisable(PFSTATE_ENVTXPROG);
	    pfDisable(PFSTATE_VTXPROG);
	    pfDisable(PFSTATE_ENFRAGPROG);
	    pfDisable(PFSTATE_FRAGPROG);
	    pfOverride(PFSTATE_ENSHADPROG | PFSTATE_SHADPROG |
 		       PFSTATE_ENVTXPROG  | PFSTATE_VTXPROG  |
		       PFSTATE_ENFRAGPROG | PFSTATE_FRAGPROG, 1);
	} else
 	    pfOverride(PFSTATE_ENSHADPROG | PFSTATE_SHADPROG | 
		       PFSTATE_ENVTXPROG  | PFSTATE_VTXPROG  |
		       PFSTATE_ENFRAGPROG | PFSTATE_FRAGPROG, 0);
...
localPostDraw(pfChannel *chan, void *data)
{
    ...
    if (ViewState->snapFrame == pfGetFrameCount())
    ...
    {
    	pfCompositor *cComp = pfGetPipeCompositor(cPipe);
    	pfCompositor *mComp = pfGetPipeCompositor(mPipe);

    	if ((cComp!=NULL) && (cComp==mComp))
    	{
	    ...
	    if ((pfuGetCursorType() == PFU_CURSOR_2D) && ViewState->mouse.inWin && 
    	    	    !pfuInGUI( PF_MAX2(0,ViewState->mouse.xpos-8), ViewState->mouse.ypos))
	    {
	...
	if(ViewState->viewportOutline) /* for debugging compositor support */
        {
            float color[8][4] = { { 1.0, 0.0, 0.0, 1.0 },  /* 0, red */
                                  ...
                                  { .85, .55, 0.1, 1.0 }  /* 7, orangy */
                                };


            int pipe = pfGetPipeNum();
    	    pfPipe* p = pfGetChanPipe(chan);

    	    pfCompositor* comp = pfGetPipeCompositor(p);
    	    int comp_child_index = pfGetPipeCompositorChildIndex(p);

    	    ScreenArea vp, chanVp;
    	    
	    int lightEnabled = glIsEnabled(GL_LIGHTING);
	    int texEnabled = glIsEnabled(GL_TEXTURE_2D);

    	    pfGetChanOutputViewport(chan, &(chanVp.left), &(chanVp.right), &(chanVp.bottom), &(chanVp.top));
    		...
    		float delta = 0.001f;   
    	    	...
    	    	pfGetCompositorChildViewport( comp, comp_child_index,
    	    	     &(vp.left), &(vp.right), &(vp.bottom), &(vp.top));

     	    	vp.left = PF_MAX2( vp.left, chanVp.left ) + delta;
    	    	vp.right = PF_MIN2( vp.right, chanVp.right ) - delta;
    	    	vp.bottom = PF_MAX2( vp.bottom, chanVp.bottom ) + delta;
    	    	vp.top = PF_MIN2( vp.top, chanVp.top ) - delta;
   	    }
	    ...
	    glDisable(GL_TEXTURE_2D);
            ...
            glPushMatrix();
            glLoadIdentity();
            glOrtho( chanVp.left, chanVp.right, chanVp.bottom, chanVp.top, 0.0, 1.0 );

            glMatrixMode( GL_MODELVIEW );
            glPushMatrix();
            glLoadIdentity();

    	    /* use (pipe % 8) below since color array has 8 entries */
            glColor4fv(color[pipe%8]);   

            glLineWidth( 12.0 );

            glBegin(GL_LINE_STRIP);
            glVertex2f( vp.left, vp.bottom );
            glVertex2f( vp.right, vp.bottom );
            glVertex2f( vp.right, vp.top );
            glVertex2f( vp.left, vp.top );
            glVertex2f( vp.left, vp.bottom );
            glEnd();
...
void
updateSmallFeatureCulling(float size)
{
    int c;
    ...
    if(size == 0)
    {
	ViewState->cullMode &= ~PFCULL_PROGRAM;
    }
    else
    {
    	int n;
	...
	for(c=0; c<n; c++)
	{
	    cullPg = pfGetChanCullProgram(ViewState->chan[c]);
	    pfCullProgramNumPolytopes(cullPg, 0);
	    pfCullProgramResetPgm(cullPg, 
				  PFCULLPG_GEOSET_PROGRAM | PFCULLPG_NODE_PROGRAM);
	    pfCullProgramAddPgmOpcode(cullPg, 
				      PFCULLPG_TEST_SMALLER_THAN, size);
	    pfCullProgramAddPgmOpcode(cullPg, 

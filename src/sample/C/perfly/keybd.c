...
printCommands(void)
{
    ...
    fprintf(stderr, " F5      toggles colored viewport outlines\n");
    fprintf(stderr, " F6      cycles between the various supported composition modes\n");
    fprintf(stderr, " F7      toggles dynamic load balancing On/Off\n");
    fprintf(stderr, " F12     Print current view position to stderr\n");
    fprintf(stderr, "  a      Toggle shader programs, vtx & frag progs\n");
...
cycleCompositorMode(void)
{
    ...
    pfCompositor * master_comp;
    int i, n = pfGetNumCompositors();

    if (n == 0)
    	return;

    master_comp = pfGetCompositor(0);
    cur_mode = pfGetCompositorMode( master_comp, PFCOMP_COMPOSITION_MODE);

    if (cur_mode == PFCOMP_COMPMODE_HORIZ_STRIPES)
    	new_mode = PFCOMP_COMPMODE_VERT_STRIPES;
    else if (cur_mode == PFCOMP_COMPMODE_VERT_STRIPES)
    	new_mode = PFCOMP_COMPMODE_LEFT_TILES;
    else if (cur_mode == PFCOMP_COMPMODE_LEFT_TILES)
    	new_mode = PFCOMP_COMPMODE_RIGHT_TILES;
    else if (cur_mode == PFCOMP_COMPMODE_RIGHT_TILES)
    	new_mode = PFCOMP_COMPMODE_BOTT_TILES;
    else if (cur_mode == PFCOMP_COMPMODE_BOTT_TILES)
    	new_mode = PFCOMP_COMPMODE_TOP_TILES;
    else if (cur_mode == PFCOMP_COMPMODE_TOP_TILES)
    	new_mode = PFCOMP_COMPMODE_ANTIALIAS;
    else if (cur_mode == PFCOMP_COMPMODE_ANTIALIAS)
    	new_mode = PFCOMP_COMPMODE_HORIZ_STRIPES;

    for (i=0; i<n; i++)
    {
    	pfCompositor *comp = pfGetCompositor(i);
    	pfCompositorMode( comp, PFCOMP_COMPOSITION_MODE, new_mode );
    }
}
...
toggleShaders(void)
{
  ViewState->shaders = !ViewState->shaders;
  updateWidget(GUI_SHADERS, ViewState->shaders);
}
...
static int theresAClipTexture(void)
{
	...
	if (pfGetPipeNumMPClipTextures(ViewState->pipe[i]) > 0)
	    return 1;
    return 0;
}
...
processKeybdInput(void)
{
	...
	if (events->devCount[dev] > 0)
	{
		...
		for (i = 0; i < events->numKeys; i++)
		{
			...
			case 'a': /* toggle enabling of pfShaderPrograms */
  			    toggleShaders();
			    break;
		...
		master_comp = pfGetCompositor(0);
		cur_mode = pfGetCompositorMode( master_comp, PFLOAD_BALANCE);
    	    	new_mode = !cur_mode;

		for (i=0; i<n; i++)
		{
    		    pfCompositor *comp = pfGetCompositor(i);
    		    pfCompositorMode( comp, PFLOAD_BALANCE, new_mode );
		}
		break;
    	    }

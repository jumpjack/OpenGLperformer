...
static int vehicleDCSAppFunc(pfTraverser *trav, void *data)
{
    ...
    pfuPath *HPRpath = pathAndTime->HPRpath;
    ...
    if(HPRpath)
    {
      ...
      pfuFollowPath(HPRpath, thisTime - prevTime, coord.hpr, dummy);
    }
...
static int vehicle(FILE *fp, int execute)
{
    ...
    strcpy(objnameBuf, pathnameBuf);
    strcat(objnameBuf, ".hpr");
    thisPathAndTime->HPRpath = pfuNewPath();

    level = pfGetNotifyLevel();
    pfNotifyLevel(PFNFY_ALWAYS);
    if (pfuAddFile(thisPathAndTime->HPRpath, objnameBuf) < 0)
    {
       pfDelete(thisPathAndTime->HPRpath);
       thisPathAndTime->HPRpath = NULL;
    }
    pfNotifyLevel(level);

...
int main(int argc, char *argv[])
{
    ...
    char toolpath[256]; /* path to cliptexture tools */
    ...
    /* Loaction of Performer tools directory */
#ifdef __linux__
    {
	char *pfroot = getenv("PFROOT");
	
	if (pfroot == NULL)
	    sprintf(toolpath, "/usr/share/Performer/src/tools/");
	else
	    sprintf(toolpath, "%s/usr/share/Performer/src/tools/", pfroot);
    }
#else
    toolpath[0] = '\0';
#endif
	...
	for(i = 0; i < maxtiles; i++) {
		...
		sprintf(cmd, "%ssubimg %s %s %d %d %d %d",
			toolpath, fname, outfile, 
			i * tilesize,
			(i + 1) * tilesize - 1,
			j * tilesize,
			(j + 1) * tilesize - 1);
	...
	for(i = 0;i < maxtiles; i++) {
		...
		*/
		...
		sprintf(cmd, "%sizoom %s tmp_ll.rgb 0.5 0.5", toolpath, infile);
		printf("%s\n", cmd);
		system(cmd);

		sprintf(infile,  format, clipsize, j * 2 , i * 2 + 1);
		sprintf(cmd, "%sizoom %s tmp_lr.rgb 0.5 0.5", toolpath, infile);
		printf("%s\n", cmd);
		system(cmd);

		sprintf(infile,  format, clipsize, j * 2 + 1 , i * 2);
		sprintf(cmd, "%sizoom %s tmp_ul.rgb 0.5 0.5", toolpath, infile);
		printf("%s\n", cmd);
		system(cmd);

		sprintf(infile,  format, clipsize, j * 2 + 1 , i * 2 + 1);
		sprintf(cmd, "%sizoom %s tmp_ur.rgb 0.5 0.5", toolpath, infile);
		printf("%s\n", cmd);
		system(cmd);
    ...
    */
	...
	sprintf(outfile, format, clipsize/2, 0, 0);
        sprintf(cmd, "%sizoom %s %s 0.5 0.5", toolpath, infile, outfile);
	printf("%s\n", cmd);
        system(cmd);
    }
